-- 06_Join_Exam.sql
-- 조인 연습문제
-- 1) = 조인을 이용해서 SCOTT 사원의 (ENAME) 부서번호와(DNO) 부서이름(DNAME)을 출력하세요
...
-- 사원테이블 : EMPLOYEE
-- 부서테이블 : DEPARTMENT 
SELECT EMP.ENAME, EMP.DNO, DEP.DNAME, DEP.LOC 
FROM EMPLOYEE EMP
JOIN DEPARTMENT DEP 
ON (EMP.DNO = DEP.DNO);
...
FROM EMPLOYEE EMP
JOIN DEPARTMENT DEP ON (EMP.DNO = DEP.DNO)
AND DEP.DNO IN (10);
...
JOIN DEPARTMENT DEP
ON (EMP.DNO = DEP.DNO)
GROUP BY EMP.DNO, DEP.DNAME, EMP.SALARY;
...
-- 부서테이블 : DEPARTMENT
-- 힌트) HAVING 사용
SELECT EMP.JOB, SUM(EMP.SALARY), DEP.DNAME
FROM EMPLOYEE EMP
JOIN DEPARTMENT DEP
ON(EMP.DNO = DEP.DNO)
